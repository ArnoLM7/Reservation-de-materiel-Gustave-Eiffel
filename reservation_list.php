...
if (!$_SESSION['connected_user']) {
    ...
    $query = "SELECT material.name, reservation.begin_date, reservation.end_date, reservation.status, reservation.id_material, reservation.id FROM `reservation` 
    JOIN material ON material.id = reservation.id_material WHERE 1";
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            <tr>
                ...
                <td><?= '<a href="details_reservation.php?id='. $id . '"><div>details</div></a>' ?></td>
            </tr>
        <?php
        }
    }
